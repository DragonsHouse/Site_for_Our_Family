...
describe('FamilyHubApp auth routing source contract', () => {
  it('uses the normalized routers and dedicated screens without rendering outcome.message directly', async () => {
    ...
    assert.match(source, /BirthdayOnboardingScreen/u);
    assert.match(source, /BirthdaySuccessScreen/u);
    assert.match(source, /StaticIdOnboardingScreen/u);
    assert.match(source, /ChangePasswordScreen/u);
    assert.match(source, /OAuthLoadingScreen/u);
    assert.match(source, /OAuthSuccessScreen/u);
    assert.match(source, /AuthOutcomeScreen/u);
    assert.match(source, /birthday-date-input/u);
    assert.match(source, /dh-initiation-sound/u);
    assert.match(source, /min=\{BIRTHDAY_MIN_DATE\}/u);
    assert.match(source, /max=\{maxDate\}/u);
    assert.match(source, /result\.code === 'too_early'/u);
    assert.match(source, /result\.code === 'future'/u);
    assert.match(source, /result\.code === 'required'/u);
    assert.match(source, /birthdayErrorMessage\(error\)/u);
    ...
    assert.match(source, /dh-initiation-success/u);
    assert.match(source, /birthday-success-heading/u);
  ...
  it('keeps onboarding audio optional and cleaned up by source contract', async () => {
    ...
    assert.match(screen, /dh-initiation-sound/u);
    assert.match(screen, /audio\.enabled && audio\.state === 'playing'/u);
    assert.match(screen, /audio\.toggle\(\)/u);
