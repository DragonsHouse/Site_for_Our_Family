...
describe('Dragon Ember Gate entrance frame composition', () => {
  it('uses the provided PNG as a frame with a form region anchored to the arch opening', () => {
    [
      ...
      'dh-ember-gate-scene',
      'dh-ember-gate-frame',
      'dh-ember-gate-arch-opening',
      'dh-ember-gate-portal',
    ...
    assert.match(emberGateSource, /useFamilyAssetUrl\('login_portal_background'\)/);
  ...
  it('does not redraw, erase or replace the frame artwork with CSS arch layers', () => {
    ...
    assert.match(emberGateStyles, /\.dh-ember-gate-frame\s*\{[^}]*object-fit:\s*cover/);
    assert.match(emberGateStyles, /\.dh-ember-gate-frame\s*\{[^}]*object-position:\s*center center/);
    ...
    assert.match(emberGateSource, /horizontalOffset = \(viewportWidth - renderedWidth\) \/ 2/);
    assert.match(emberGateSource, /verticalOffset = \(viewportHeight - renderedHeight\) \/ 2/);
    assert.match(emberGateStyles, /\.dh-ember-gate-arch-opening\s*\{[\s\S]*?left:\s*var\(--ember-gate-opening-left\)/);
    ...
    assert.match(emberGateStyles, /\.dh-ember-gate-arch-opening\s*\{[\s\S]*?z-index:\s*1/);
    assert.match(emberGateStyles, /\.dh-ember-gate-arch-opening\s*\{[\s\S]*?pointer-events:\s*none/);
    assert.match(emberGateStyles, /\.dh-ember-gate-form-panel\s*\{[\s\S]*?pointer-events:\s*auto/);
    assert.match(emberGateStyles, /\.dh-ember-gate-portal\s*\{[\s\S]*?z-index:\s*0/);
    assert.match(emberGateStyles, /\.dh-ember-gate-portal\s*\{[\s\S]*?clip-path:\s*polygon/);
    assert.doesNotMatch(emberGateStyles, /transform:\s*scale/);
    assert.doesNotMatch(emberGateStyles, /backdrop-filter/);
    assert.doesNotMatch(emberGateStyles, /filter\s*:/);
  });

  it('adds lightweight portal motion and opt-in ambient sound without generated art', () => {
    ...
    assert.match(emberGateStyles, /@keyframes dh-portal-fire-slow/);
    assert.match(emberGateStyles, /@keyframes dh-portal-smoke-drift/);
    assert.match(emberGateStyles, /@keyframes dh-portal-embers-rise/);
    assert.match(emberGateStyles, /prefers-reduced-motion/);
  });
